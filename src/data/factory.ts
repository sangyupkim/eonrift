...
export const BUILDINGS: Record<BuildingType, BuildingDef> = {
  ...
  infuser: { type: 'infuser', name: '마력 주입기', power: 12, color: 0x8a6aff, cost: { iron_ore: 4, copper_ore: 4 }, description: '주괴에 마력을 불어넣는다.', blueprint: { gold: 800, items: { copper_ingot: 3, plank: 3 } } },
...
export const RECIPES: Recipe[] = [
  ...
  { id: 'plank', machine: 'crusher', inputs: { wood: 1 }, output: 'plank', count: 2, tier: 1, time: 15 },
  { id: 'plank_redpine', machine: 'crusher', inputs: { redpine_wood: 1 }, output: 'plank', count: 3, tier: 2, time: 15 },
  { id: 'plank_frost', machine: 'crusher', inputs: { frost_wood: 1 }, output: 'plank', count: 3, tier: 3, time: 15 },
  { id: 'plank_crystal', machine: 'crusher', inputs: { crystal_wood: 1 }, output: 'plank', count: 4, tier: 4, time: 15 },
  { id: 'plank_iron', machine: 'crusher', inputs: { iron_wood: 1 }, output: 'plank', count: 4, tier: 5, time: 15 },
  { id: 'plank_flame', machine: 'crusher', inputs: { flame_wood: 1 }, output: 'plank', count: 5, tier: 6, time: 15 },
  { id: 'plank_dim', machine: 'crusher', inputs: { dim_wood: 1 }, output: 'plank', count: 6, tier: 7, time: 15 },
  { id: 'mana_copper', machine: 'infuser', inputs: { copper_ingot: 1, essence_low: 1 }, output: 'mana_copper', count: 1, tier: 1, time: 25 },
  { id: 'mana_iron', machine: 'infuser', inputs: { iron_ingot: 1 }, output: 'mana_iron', count: 1, tier: 2, time: 30 },
  { id: 'mana_gold', machine: 'infuser', inputs: { gold_ingot: 1 }, output: 'mana_gold', count: 1, tier: 3, time: 40 },
  { id: 'mana_titanium', machine: 'infuser', inputs: { titanium_ingot: 1 }, output: 'mana_titanium', count: 1, tier: 5, time: 50 },
...
export function upgradeBlueprintCost(type: BuildingType, level: number): { gold: number; items: Record<string, number> } {
  ...
  return { gold: 400 * level * heavy, items: { [LEVEL_INGOT[level - 1]]: 3 * heavy, plank: 4 * level } };
}

/** 설치된 건물을 Lv.L로 올리는 비용 (도면이 있어야 한다) */
export function buildingUpgradeCost(type: BuildingType, level: number): Record<string, number> {
  const heavy = type === 'assembler' || type === 'generator' ? 2 : 1;
  return { [LEVEL_INGOT[level - 1]]: 5 * heavy, plank: 5 * level };
}
