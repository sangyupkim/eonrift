...
export const BUILDINGS: Record<BuildingType, BuildingDef> = {
  ...
  infuser: { type: 'infuser', name: '마력 주입기', power: 12, color: 0x8a6aff, cost: { iron_ore: 6, copper_ore: 4 }, description: '주괴·판자에 마력을 불어넣어 마력 금속·마력 판자를 만든다.', blueprint: { gold: 1200, items: { copper_ingot: 5, plank: 5 } } },
  assembler: { type: 'assembler', name: '조립기', power: 10, color: 0x4a9a8a, cost: { copper_ore: 4, wood: 4 }, description: '고른 설계대로 여러 재료를 조립한다.', blueprint: { gold: 600, items: { copper_ingot: 3, plank: 3 } } },
...
export const BUILD_ORDER: BuildingType[] = ['generator', 'wire', 'belt', 'box', 'workbench', 'healer', 'smelter', 'crusher', 'assembler', 'infuser', 'alchemy', 'splitter'];
...
/** 레벨을 올릴 수 있는 건물 */
export const UPGRADABLE: BuildingType[] = ['generator', 'smelter', 'crusher', 'assembler', 'infuser', 'alchemy'];
...
export function upgradeBlueprintCost(type: BuildingType, level: number): { gold: number; items: Record<string, number> } {
  const heavy = type === 'infuser' || type === 'generator' ? 2 : 1;
