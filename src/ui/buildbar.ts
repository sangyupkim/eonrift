...
export class BuildBar {
  ...
  show(unlocked: (t: BuildingType) => boolean): void {
    const tools = BUILD_ORDER.filter(unlocked)
      .map((t) => {
        const d = BUILDINGS[t];
        const img = buildingThumb(t);
        return `<button class="tool" data-tool="${t}" style="--c:${hex(d.color)}">${img ? `<img src="${img}" alt="">` : '<i></i>'}<span>${d.name}</span></button>`;
      })
      .join('');
    this.root.innerHTML = `
      <div class="build-top">
  ...
  private refreshInfo(): void {
    if (this.tool === 'remove') this.info.innerHTML = '<b>철거</b> · 누른 건물을 없애고 재료를 돌려받습니다';
      ...
      const cost = Object.entries(d.cost)
        ...
        .join(', ');
      const drag = this.tool === 'belt' || this.tool === 'wire' ? ' · 끌어서 이어 깔기' : ` · 방향 ${DIR_ARROWS[this.dir]} · 누른 채 움직여 위치 잡기`;
